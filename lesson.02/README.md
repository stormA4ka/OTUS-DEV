...
Для тестирования вашей программы с использованием `pytest`, можно написать несколько тестов, которые проверяют различные аспекты работы классов `User` и `Book`. Вот некоторые идеи для тестов:

### Тесты для класса `User`

1. **Тест на создание пользователя с корректными данными:**
   - Проверка, что пользователь создается с правильными значениями.

2. **Тест на создание пользователя с некорректным номером телефона:**
   - Проверка, что при попытке создать пользователя с некорректным номером телефона (не числом) выбрасывается исключение `WrongInt`.

3. **Тест на автоматическое присвоение ID:**
   - Проверка, что при создании нескольких пользователей ID присваиваются последовательно.

### Тесты для класса `Book`

1. **Тест на чтение пользователей из файла:**
   - Проверка, что пользователи корректно читаются из файла и создаются объекты `User`.

2. **Тест на сохранение пользователей в файл:**
   - Проверка, что после добавления пользователя данные корректно сохраняются в файл.

3. **Тест на поиск пользователя по ID:**
   - Проверка, что метод `search_user` корректно находит пользователя по ID.

4. **Тест на удаление пользователя:**
   - Проверка, что пользователь удаляется из списка и данные сохраняются в файл.

5. **Тест на редактирование пользователя:**
   - Проверка, что данные пользователя корректно обновляются и сохраняются в файл.

6. **Тест на добавление пользователя:**
   - Проверка, что новый пользователь корректно добавляется в список и данные сохраняются в файл.

7. **Тест на получение информации о книге:**
   - Проверка, что метод `about_book` корректно возвращает количество записей и размер файла.

### Примеры тестов

Вот примеры некоторых тестов:
...
### Примечания:
1. **Фикстуры:** В примере используется фикстура `tmpdir` для создания временного файла, который будет использоваться в тестах. Это позволяет изолировать тесты друг от друга.
2. **Мокинг:** Если необходимо, можно использовать мокинг для замены реальных вызовов функций на фиктивные (например, для тестирования исключений).
3. **Патчинг:** Если нужно изменить поведение какой-либо функции (например, `datetime.datetime.now`), можно использовать патчинг с помощью `unittest.mock.patch`.

Эти тесты должны покрыть основные аспекты работы вашей программы и помочь в отладке и поддержке кода.
