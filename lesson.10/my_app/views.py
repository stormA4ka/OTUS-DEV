...
def task_list(request):
    tasks = Task.objects.all().order_by(
        "-task_date"
    )  # Получаем все заявки, сортируем по дате (новые сверху)
...
def add_task(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            task = form.save()
            log_task_action.delay(
                "Создание", task.task_id
...
def edit_task(request, task_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            log_task_action.delay(
                "Редактирование", task.task_id
            )  # Логирование редактирования задачи
...
class TaskUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        log_task_action.delay(
            "Редактирование", self.object.task_id
        )  # Логирование редактирования задачи
        return response
...
class TaskDeleteView(DeleteView):
    model = Task  # Указываем модель
    template_name = (
        "my_app/task_confirm_delete.html"  # Шаблон для подтверждения удаления
    )
    pk_url_kwarg = "task_id"  # Имя параметра в URL
